...
fromEvent(document, 'click')
  .pipe(
    ...
    tap(x => log(x)),
    switchMap(() => books$),
    // switchMap(() => concat(library$, books$))
    tap(x => log(x)),
...
` + DEMO_PURPOSES,
  ...
  'error-catch': {
    ...
    timeWindow: 10000,
    code: 
`const { fromEvent, of, forkJoin, throwError } = Rx;
const { scan, tap, switchMap, delay, catchError, retry, finalize } = RxOperators;

const library$ = of('Library list')
  .pipe(
    delay(1000 + Math.random() * 2000),
    switchMap(() => throwError('Timeout error'))
  );

library$.subscribe(null, x => log('Hubo un error: ' + x));
library$
...
library$
  .pipe(
    // finalize(() => log('ALWAYS')),
    catchError((x) => of('Otro observable'))
  )
*/
